...
## Abordagem técnica
- Nova página `dashboard/paginas/chat.py`, ao lado de `dashboard/paginas/metricas.py` (F7) — roteadas por um menu lateral (`st.Page`/`st.navigation`) em `dashboard/app.py`. **Revisão**: a primeira versão usava a descoberta automática de `dashboard/pages/` (convenção antiga do Streamlit, sem menu com rótulos/ícones explícitos); trocado por `st.navigation` a pedido do usuário, para ter um menu de verdade para alternar entre as páginas — ver log em implements.md.
