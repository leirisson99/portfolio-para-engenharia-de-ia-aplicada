...
- **Ajuste do usuário (2026-08-28)**: `dashboard/app.py` passou a chamar `load_dotenv(...)` antes de montar a navegação — `streamlit run` não carrega `.env` sozinho, e a página de chat precisa de `OPENROUTER_API_KEY` em tempo de execução real (fora de teste, onde `CHAT_MOCK_PIPELINE=1` evita essa dependência). Verificado que os 15 testes de `tests/dashboard/` continuam passando (o `monkeypatch.setenv` dos testes tem precedência, já que `load_dotenv` não sobrescreve variável já definida por padrão).
